  ...
  if (currentPlayer === canvas1) {
    ...
    if (
      ...
      leftScoreTotal.innerText =
        Number(leftScoreThisTurn.innerText) + Number(leftScoreTotal.innerText);
    ...
    if (
      Number(rightScoreTotal.innerText) +
        Number(rightScoreThisTurn.innerText) >=
      100
    ) {
      console.log("cavnas2 player has scored at or over 100");

      rightScoreTotal.innerText =
        Number(rightScoreThisTurn.innerText) +
        Number(rightScoreTotal.innerText);
...
holdDieBtn.addEventListener("click", () => {
  ...
  if (hasPlayerRolled) {
    ...
    alert(
      `${currentPlayerText.innerText} needs to roll first before holding !`
    );
  }
});
